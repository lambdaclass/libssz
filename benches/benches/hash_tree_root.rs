...
fn htr_primitives(c: &mut Criterion) {
    ...
    group.bench_function("u8", |b| {
        b.iter(|| black_box(0xABu8).hash_tree_root(&libssz_merkle::Sha2Hasher))
    });
    group.bench_function("u16", |b| {
        b.iter(|| black_box(0x1234u16).hash_tree_root(&libssz_merkle::Sha2Hasher))
    });
    group.bench_function("u32", |b| {
        b.iter(|| black_box(0x1234_5678u32).hash_tree_root(&libssz_merkle::Sha2Hasher))
    });
    group.bench_function("u64", |b| {
        b.iter(|| black_box(0x1234_5678_9abc_def0u64).hash_tree_root(&libssz_merkle::Sha2Hasher))
    });
    group.bench_function("u128", |b| {
        b.iter(|| black_box(u128::MAX).hash_tree_root(&libssz_merkle::Sha2Hasher))
    });
    group.bench_function("bytes32", |b| {
        b.iter(|| black_box([0xabu8; 32]).hash_tree_root(&libssz_merkle::Sha2Hasher))
    });
    group.finish();
}
...
fn htr_vec_u64(c: &mut Criterion) {
    ...
    for &size in &[100, 1_000, 100_000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &data, |b, data| {
            b.iter(|| black_box(data).hash_tree_root(&libssz_merkle::Sha2Hasher));
        });
    }
    group.finish();
}
...
fn htr_list_validator(c: &mut Criterion) {
    ...
    for &size in &[100, 1_000, 10_000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &list, |b, list| {
            b.iter(|| black_box(list).hash_tree_root(&libssz_merkle::Sha2Hasher));
        });
    }
    group.finish();
}
...
fn htr_vector_bytes32(c: &mut Criterion) {
    ...
    group.bench_function("100", |b| {
        b.iter(|| black_box(&v100).hash_tree_root(&libssz_merkle::Sha2Hasher))
    });
    group.throughput(Throughput::Bytes((1000 * 32) as u64));
    group.bench_function("1000", |b| {
        b.iter(|| black_box(&v1000).hash_tree_root(&libssz_merkle::Sha2Hasher))
    });
    group.finish();
}
        ...
        match bits {
            64 => {
                ...
                group.bench_with_input(BenchmarkId::new("bitlist", bits), &bl, |b, bl| {
                    b.iter(|| black_box(bl).hash_tree_root(&libssz_merkle::Sha2Hasher))
                });
                let bv = make_bitvector::<64>();
                group.bench_with_input(BenchmarkId::new("bitvector", bits), &bv, |b, bv| {
                    b.iter(|| black_box(bv).hash_tree_root(&libssz_merkle::Sha2Hasher))
                });
            }
            512 => {
...
fn htr_containers(c: &mut Criterion) {
    ...
    group.bench_function("validator", |b| {
        b.iter(|| black_box(&validator).hash_tree_root(&libssz_merkle::Sha2Hasher))
    });
    group.bench_function("beacon_block_header", |b| {
        b.iter(|| black_box(&header).hash_tree_root(&libssz_merkle::Sha2Hasher))
    });
    group.finish();
}
...
fn htr_consensus_containers(c: &mut Criterion) {
    ...
    group.bench_function("pending_attestation", |b| {
        b.iter(|| black_box(&pending_attestation).hash_tree_root(&libssz_merkle::Sha2Hasher))
    });
    group.finish();
}
